...
function dragStartHandler(e) {
    ...
    let animalType = this.dataset.animalName;
    this.classList.add('dragged', 'drag-feedback');
    e.dataTransfer.setData(`type/dragged-${animalType}`, animalType);
    e.dataTransfer.setData(`type/dragged-card`, animalType);
    e.dataTransfer.setData("text/plain", this.querySelector('img').getAttribute('alt'));
...
function dropZoneEnterHandler(e) {
    ...
    if (canDropHere(e)) {
        ...
        e.preventDefault();
    } else if (this.classList.contains('card-slot')) {
...
function dropZoneDropHandler(e) {
    ...
    if (winCheck()) {
        document.querySelector('body').classList.add('game-solved');
    }
